...
importsRouter.post('/upload', upload.single('image'), async (req, res, next) => {
  try {
...
importsRouter.get('/:importId/review', async (req, res, next) => {
  try {
...
importsRouter.post('/:importId/confirm', async (req, res, next) => {
  try {
